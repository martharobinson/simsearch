...
ROOT_DATA_PATH = Path(__file__).parent.parent / "data/In-shop Clothes Retrieval Benchmark"
...
def search_api(image, k, endpoint_url):
    ...
    for r in results:
        try:
            ...
            img = None
        images.append(img)
        captions.append(f"{os.path.basename(r['filename'])} (score: {r['score']:.2f})")
...
METHODS = [
    {"name": "CLIP-FAISS", "endpoint": "http://localhost:8000/search"},
    # Add more methods here
]

# Load example images from query split
def get_example_images(n=8):
...
with gr.Blocks(css=custom_css) as demo:
    gr.Markdown("# Image Similarity Search: Multi-Method Comparison\n\n**Score = Cosine Similarity (-1 to 1)**")
    with gr.Row():
    ...
    for method in METHODS:
        ...
        with gr.Row():
            gallery = gr.Gallery(label=f"Results ({method['name']}, Cosine Similarity Score)", columns=5, rows=1, height=140, object_fit="contain")
        ...
        for method in METHODS:
            ...
            for caption in captions:
                if "(score:" in caption:
                    ...
                    label_dict[f"{label} (score: {score:.2f}, dataset: {dataset})"] = score
            outputs.append((images, label_dict))
        return [o[0] for o in outputs] + [o[1] for o in outputs]
    search_btn.click(run_search, inputs=[query_img, k_input], outputs=[r[0] for r in result_rows] + [r[1] for r in result_rows])
