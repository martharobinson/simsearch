...
def main(data_root, split, batch_size, output):
    """Generate CLIP image embeddings for DeepFashion dataset and save to file."""
    ...
    for batch in tqdm(dataloader, desc=f"Embedding {split} images", total=num_batches):
        ...
        embeds = generate_clip_embeddings(model, processor, images=images, device="mps")
        image_embeds = embeds["image_embeds"].cpu().numpy()
